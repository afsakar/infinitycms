...
<?php else: ?>
    ...
    <div class="table-responsive">
        <table id="responsive-datatable" data-plugin="DataTable" class="table table-hover pictures_list" cellspacing="0" width="100%">
            ...
            <tbody class="sortable" data-url="<?php echo base_url("courses/imageRankSetter")?>">
            <?php foreach ($itemImages as $image): ?>
                <tr id="ord-<?=$image->id?>">
                    ...
                    <td>
                        <input class="isActive" data-url="<?php echo base_url("courses/imageIsActiveSetter/$image->id")?>" type="checkbox" data-switchery data-color="#25D366" <?=$image->isActive == 1 ? 'checked' : null?>/>
                    </td>
                    <td>
                        <button data-url="<?php echo base_url("courses/deleteImage/$image->id/$image->courses_id")?>" class="btn btn-danger btn-sm btn-outline remove-btn"><i class="fa fa-trash"></i> Sil</button>
                    </td>
                </tr>
            <?php endforeach; ?>
            </tbody>
